    ...
    sts1 = controller.scannedDataStream.listen((scanData) async {
      ...
      if (scanData != null && scanData.code != null && scanData.code != "") {
        ...
        if (chargerInfo != null && chargerInfo.length > 0) {
          final retVal =
              await FirebaseService.instance.isChargerExist2(chargerInfo);
          ...
          if (retVal["exists"] == true) {
            ...
            controller.dispose();
            await Get.find<GMapController>()
                .prepareCharger(retVal["locationID"], chargerInfo, "1");
            //controller.resumeCamera();
          } else {
            await Get.to(DisplayMsgPage(
              txt: "Invalid Code",
              msgType: DisplayMsgType.Error,
            ));
            controller.resumeCamera();
          }
        }
            ...
            builder: (logic) {
              if (logic.flashStatus.value) {
                return Ink(
                  ...
                  child: IconButton(
                    icon: Icon(
                      Icons.flashlight_off_outlined, //เปลี่ยน Icon LIGHT_OFF
                ...
                return IconButton(
                  ...
                  icon: Icon(
                    Icons.flashlight_on_outlined, //เปลี่ยน Icon LIGHT_ON
                    color: _theme.darkColor,
                    size: _sectionHeight / 4,
                  ),
                  onPressed: () {
...
class LightButtonController extends GetxController {
  ...
  void onInit() async {
    ...
    icon = Icon(Icons.flashlight_off_outlined).obs;
